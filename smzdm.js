...
let smzdmCommit = cookieSess => {
  ...
  let options = {
    url:
      'https://zhiyou.smzdm.com/user/comment/ajax_set_comment?callback=jQuery111006551744323225079_' +
      new Date().getTime() +
      '&type=3&pid=' +
      pId +
      '&parentid=0&vote_id=0&vote_type=&vote_group=&content=' +
      encodeURI(commitList[Math.floor(Math.random() * commitList.length)]) +
      '&_=' +
      new Date().getTime(),
    type: 'GET',
  };
    ...
    options.callback = (data, _setCookie) => {
      try {
        ...
        if (data.indexOf('"error_code":0') != -1) {
          ...
          mailSend(
            '什么值得买发送【评论报错】',
            `时间: ${new Date().Format('yyyy-MM-dd hh:mm:ss')}  <br/>用户: ${cookieName} <br/>错误内容: ${ascii2native(data)}`,
          );
        }
      } catch (error) {
        console.log(error);
        ...
        mailSend(
          '什么值得买发送【评论报错】',
          `时间: ${new Date().Format('yyyy-MM-dd hh:mm:ss')}  <br/>用户: ${cookieName} <br/>错误内容: ${ascii2native(error)}`,
        );
      } finally {
      }
    };
